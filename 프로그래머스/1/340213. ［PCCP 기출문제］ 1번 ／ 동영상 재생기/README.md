...
### ✅ 접근 방법
1. 시간 단위 통일
   - `"MM:SS"` -> 초 단위로 바꿔서 계산(`MM*60 + SS`)
2. 오프닝 스킵 처리
   - 현재 시간이 오프닝 구간 안에 있으면 오프닝 끝으로 이동
   - 명령 처리 후에도 오프닝 시간 안에 들어갈 수 있으므로 재처리
3. 명령 처리
   - `prev` -> 현재 시간 - 10, 최소 0초 제한
   - `next` -> 현재 시간 + 10, 최대 영상 길이 제한
4. 최종 시간 출력
   - 계산이 끝나면 다시 `"MM:SS"` 형태로 변환
